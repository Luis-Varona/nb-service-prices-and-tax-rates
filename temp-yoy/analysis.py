...
MODES = [
    ("reg_rate", False, False),
    ("reg_rate_mpsa", False, True),
    ("reg_diff", True, False),
    ("reg_diff_mpsa", True, True),
]
...
def main() -> None:
    ...
    for prefix, diff, mpsa_only in MODES:
        title = f"YearDiff={diff}, HasMuni={not mpsa_only}"
        df = load_data(diff, mpsa_only)
        results = {}
...
# %%
def load_data(diff: bool, mpsa_only: bool) -> pl.DataFrame:
    ...
    df = df_master.join(df_revs, on=[TIME_COL, ENTITY_COL], how="left")

    if mpsa_only:
        df = df.filter(pl.col("Provider_PPSA") | pl.col("Provider_MPSA"))

    df = (
        df.drop("Provider_MPSA")
        .with_columns(
